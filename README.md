...
El frontend ya incluye una pantalla administrativa dentro del modulo `CONFIGURATION` para editar configuracion visual/regional.

- Carga `GET /api/v1/company/settings`.
- Guarda con `PUT /api/v1/company/settings` y version optimista.
- Permite cancelar cambios locales.
- Muestra preview de colores, fecha/hora y moneda.
- Maneja permisos insuficientes y conflictos de version.
- Aplica el perfil runtime actualizado al tema/formato del shell despues de guardar.

Ver detalle en `docs/configuration/FRONTEND_COMPANY_SETTINGS_ADMIN.md`.

Los assets visuales por tenant ya estan controlados.

- Backend valida `logoUrl`, `faviconUrl` y `loginImageUrl`.
- Solo se aceptan URLs `https://` con host valido.
- Se rechazan `data:`, `javascript:`, URLs relativas, userinfo y fragmentos.
- El shell muestra logo runtime con fallback.
- El navegador aplica favicon runtime con fallback.
- La UI administrativa muestra preview controlada de logo, favicon e imagen de login.

Ver detalle en `docs/configuration/VISUAL_ASSETS.md`.
...
- Suite backend actual: 150 tests unitarios y 15 pruebas de integracion en `mvn -B verify`, 0 fallos.
